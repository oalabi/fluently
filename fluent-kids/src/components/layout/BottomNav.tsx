...
export function BottomNav({ lang = "yoruba" }: BottomNavProps) {
  ...
  return (
    <nav className="sticky bottom-0 z-50 border-t border-white/10 bg-fluent-purple-dark/95 px-2 py-2 backdrop-blur">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`touch-target relative flex flex-col items-center gap-0.5 rounded-xl px-2 py-1 text-xs ${
                active ? "text-fluent-yellow" : "text-white/60"
              }`}
            >
